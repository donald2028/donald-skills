...
def configured_browser(path: Path | None = None, scope: str | None = None) -> dict[str, Any]:
    ...
    if config is None:
        raise ProfileConfigError(
            ...
            f"donald-config-browser skill first. Expected config: {config_path}"
        )
    return config
...
def _cdp_target(port: int, target_id: str) -> dict[str, Any] | None:
    try:
        with urllib.request.urlopen(
...
def connect_cdp_target(port: int, target_id: str, timeout: float = 30.0) -> _CDPConnection:
    target = _cdp_target(port, target_id)
...
def call_background_page(
    ...
    if not websocket_url:
        raise ProfileConfigError(f"Chrome CDP page target {target_id} is unavailable")
    try:
        with _CDPConnection.connect(websocket_url, timeout=timeout) as connection:
...
def close_background_page(port: int, target_id: str) -> None:
    ...
    for _ in range(20):
        if _cdp_target(port, target_id) is None:
            return
        time.sleep(0.1)
...
def hide_browser_without_focus(
    config: dict[str, Any],
    port: int,
    restore_pid: int | None = None,
) -> dict[str, Any]:
    if sys.platform != "darwin":
        return {"status": "platform_default", "cdp_port": port}
    if not _cdp_version(port):
...
def frontmost_process_id() -> int | None:
    ...
    try:
        ...
        raise ProfileConfigError(f"Invalid foreground process id: {result.stdout.strip()}") from error
...
def restore_frontmost_process_if_browser_active(pid: int, port: int) -> dict[str, Any]:
    if sys.platform != "darwin":
        return {"status": "platform_default", "pid": pid}
    # Chrome may promote a CDP target to the foreground just after the command returns.
    # Let that event settle before deciding whether the previous app needs restoring.
    time.sleep(0.25)
    current_pid = frontmost_process_id()
    browser_pid = _listening_process_id(port)
